...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Karly Guard | HiperFun Guard Bot" }, status: "idle" });
...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("RANDOM").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(`Safe Code - Karly ❤️ Salvatore`)
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "Karly - Guard Koruma Sistemi" }).catch();
};
//--------------------------CEZALANDIRMA--------------------------\\
...
.setFooter(`Karly - v12 Guard Botu`))
} else { 
...
.setDescription(`${entry.executor} İsimli Yetkili ${member} İsimli Kullanıcıyı Sağ Tık Kullanarak Sunucudan Kickledi Gerekli İşlemler Yapıldı`)
.addField("Yapılan İşlem",`${entry.executor} İsimli Kullanıcı Jail'e Atıldı`)
.setFooter(`Karly - v12 Guard Botu`)).catch(err => {}); };
});
//--------------------------SAĞ TIK KİCK KORUMASI--------------------------\\
...
.addField("Yapılan İşlem",`${entry.executor} İsimli Kullanıcı Jail'e Atıldı`)
.setFooter(`Karly - v12 Guard Botu`)).catch(); 
} else { 
...
client.on("channelCreate", async channel => {
  ...
  channel.delete({reason: "Karly Kanal Koruma"});
...
client.on("channelDelete", async channel => {
  ...
  if (!entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id) || !ayarlar.channelGuard) return;
  cezalandir(entry.executor.id, "ban");
  await channel.clone({ reason: "Karly Kanal Koruma" }).then(async kanal => {
...
client.login(ayarlar.botToken).then(c => console.log(`${client.user.tag} Giriş Başarılı [Karly]`)).catch(err => console.error("Bot Giriş Yaparken Bir Hata Oluştu"));
